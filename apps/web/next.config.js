...
const DIGITAL_TWIN_HOST = process.env.DIGITAL_TWIN_URL || 'http://localhost:8011';
const EOL_HOST = process.env.EOL_PRIORITISER_URL || 'http://localhost:8010';
const CERTIN_HOST = process.env.CERTIN_REPORTER_URL || 'http://localhost:8008';
const OT_HOST = process.env.OT_CONNECTOR_URL || 'http://localhost:8009';
...
const nextConfig = {
  ...
  env: {
    ...
    NEXT_PUBLIC_DIGITAL_TWIN_URL: process.env.NEXT_PUBLIC_DIGITAL_TWIN_URL || '',
    NEXT_PUBLIC_EOL_PRIORITISER_URL: process.env.NEXT_PUBLIC_EOL_PRIORITISER_URL || '',
    NEXT_PUBLIC_CERTIN_REPORTER_URL: process.env.NEXT_PUBLIC_CERTIN_REPORTER_URL || '',
    NEXT_PUBLIC_OT_CONNECTOR_URL: process.env.NEXT_PUBLIC_OT_CONNECTOR_URL || '',
  },
  ...
  async rewrites() {
    return [
      ...
      // CNI microservices — Digital Twin, EoL Prioritiser, CERT-In Reporter, OT Connector
      {
        source: '/api/v1/twin/:path*',
        destination: `${DIGITAL_TWIN_HOST}/api/v1/twin/:path*`,
      },
      {
        source: '/api/v1/eol/:path*',
        destination: `${EOL_HOST}/api/v1/eol/:path*`,
      },
      {
        source: '/api/v1/certin/:path*',
        destination: `${CERTIN_HOST}/api/v1/report/:path*`,
      },
      {
        source: '/api/v1/ot/:path*',
        destination: `${OT_HOST}/api/v1/ot/:path*`,
      },
