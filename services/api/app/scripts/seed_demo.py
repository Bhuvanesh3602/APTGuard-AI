...
_REALISTIC_INCIDENTS: list[dict] = [
    ...
    {
        "key": "CNI-001",
        "title": "AIIMS Delhi-pattern: LockBit 3.0 ransomware targeting hospital EHR systems",
        "description": (
            "Ransomware attack on government hospital Electronic Health Record system matching "
            "the AIIMS Delhi 2022 pattern. Attackers exploited EoL Windows Server 2008 (unpatched "
            "CVE-2017-0144 EternalBlue) to move laterally from admin network to clinical systems. "
            "5,000+ patient records encrypted. CERT-In mandatory 6-hour reporting deadline triggered. "
            "Clinical operations switched to paper-based fallback."
        ),
        "severity": "critical",
        "status": "in_progress",
        "host": "AIIMS-EHR-SRV01",
        "user": "svc-ehrsystem@aiims.gov.in",
        "src_ip": "45.142.212.100",
        "tactic_ids": ["TA0001", "TA0002", "TA0008", "TA0040"],
        "technique_ids": ["T1190", "T1059.003", "T1021.002", "T1486", "T1490"],
        "tags": ["ransomware", "lockbit3", "healthcare", "cni", "aiims_pattern", "certin_cat4", "eol_windows"],
        "alerts": [
            {
                "title": "EoL-Amplified: EternalBlue exploit on Windows Server 2008 (AIIMS-EHR-SRV01) — CVSS 7.5 → 16.5 (EoL ×2.2)",
                "severity": "critical",
                "source": "AiSOC EoL Prioritiser",
                "category": "vulnerability",
                "sourcetype": "aisoc:eol:amplification",
                "process": "smb.exe",
                "ai_score": 0.98,
                "extra": {
                    ...
                    "reason": "EoL system — no patches will ever be released",
                },
            },
            {
                "title": "Sysmon: vssadmin shadow-copy deletion + bcdedit recovery disable on AIIMS-EHR-SRV01",
                "severity": "critical",
                "source": "Microsoft Sysmon",
                "category": "edr",
                "sourcetype": "XmlWinEventLog:Microsoft-Windows-Sysmon/Operational",
                ...
                "extra": {
                    ...
                    "Image": r"C:\Windows\System32\vssadmin.exe",
                    "CommandLine": "vssadmin delete shadows /all /quiet",
                    "ParentCommandLine": "bcdedit /set {default} recoveryenabled no",
                    "hostname": "AIIMS-EHR-SRV01",
                    "mitre_technique": "T1490",
                    "certin_category": "CAT-4",
                },
            },
            {
                "title": "UEBA: Anomalous bulk file encryption — 12,847 .ehr files modified in 4 minutes (baseline: 0)",
                "severity": "critical",
                "source": "AiSOC UEBA",
                "category": "ueba",
                "sourcetype": "aisoc:ueba:anomaly",
                "process": "explorer.exe",
                "ai_score": 0.99,
                "extra": {
                    "anomaly_type": "bulk_file_modification",
                    "files_modified": 12847,
                    "time_window_seconds": 240,
                    "baseline_rate": 0,
                    "deviation_score": 9.9,
                    "file_extension_written": ".lockbit3",
                    "affected_path": r"D:\EHR\PatientRecords",
                },
            },
        ],
        "playbook_run": {
            "playbook_id": "cni-healthcare-ransomware-response-v1",
            "playbook_name": "CNI Healthcare: Ransomware Response (AIIMS Pattern)",
            "status": "running",
            "context": {"incident_severity": "critical", "host": "AIIMS-EHR-SRV01", "sector": "healthcare_cni"},
            "steps": [
                ("triage", "Classify clinical vs administrative systems", "completed", {"clinical_systems_protected": True}),
                ("protect_clinical", "Block ransomware spread to clinical ward systems", "completed", {"segment_blocked": "clinical_wards", "duration_ms": 3200}),
                ("isolate_admin", "Isolate administrative EHR server", "completed", {"host": "AIIMS-EHR-SRV01", "duration_ms": 4100}),
                ("shadow_copy_check", "Emergency VSS snapshot of remaining patient records", "completed", {"records_saved": 51230}),
                ("certin_notify", "File CERT-In mandatory report CAT-4 (6-hour deadline)", "running", {"report_id": "CERTIN-CNI001-A1B2", "hours_remaining": 4.2}),
                ("notify_mo", "Notify Ministry of Health and NCIIPC", "pending", {}),
                ("activate_dr", "Activate paper-based clinical fallback (BCP-HC-001)", "pending", {}),
            ],
        },
    },
    {
        "key": "CNI-002",
        "title": "APT36 (Transparent Tribe) targeting CBSE examination infrastructure",
        "description": (
            "APT36 spear-phishing campaign against CBSE exam coordinators using Crimson RAT. "
            "Attackers exfiltrated 2.3M student examination records (Aadhaar-linked PII) before "
            "deploying ransomware 72 hours later. Attack pattern matches CERT-In Advisory CI-2024-0018. "
            "CERT-In CAT-9 data breach + CAT-4 ransomware mandatory reporting triggered."
        ),
        "severity": "critical",
        "status": "investigating",
        "host": "CBSE-EXAM-DB01",
        "user": "exam.coordinator@cbse.gov.in",
        "src_ip": "103.76.228.95",
        "tactic_ids": ["TA0001", "TA0009", "TA0010", "TA0011"],
        "technique_ids": ["T1566.001", "T1059.001", "T1071.001", "T1041", "T1560.001"],
        "tags": ["apt36", "transparent_tribe", "education", "cni", "data_exfil", "certin_cat9", "cbse_pattern"],
        "alerts": [
            {
                "title": "APT Attribution: APT36 (Transparent Tribe) — Crimson RAT C2 beacon detected (confidence 0.87)",
                "severity": "critical",
                "source": "AiSOC India APT Agent",
                "category": "threat_intel",
                "sourcetype": "aisoc:apt:attribution",
                "process": "svchost.exe",
                "ai_score": 0.96,
                "extra": {
                    "actor": "APT36",
                    "alias": "Transparent Tribe",
                    "origin": "Pakistan-linked",
                    "confidence": 0.87,
                    "matched_ttps": ["T1566.001", "T1059.001", "T1071.001", "T1105"],
                    "tool": "Crimson RAT",
                    "certin_ref": "CERT-In Advisory CI-2024-0018",
                    "predicted_next_ttps": ["T1560.001", "T1048", "T1567"],
                },
            },
            {
                "title": "DLP: Bulk export of 2.3M student examination records to external destination",
                "severity": "critical",
                "source": "AiSOC DLP",
                "category": "data-exfil",
                "sourcetype": "aisoc:dlp:exfil",
                "process": "mysqldump.exe",
                "ai_score": 0.98,
                "extra": {
                    "records_exfiltrated": 2300000,
                    "data_type": "student_pii_aadhaar_linked",
                    "destination": "103.76.228.95",
                    "destination_country": "Pakistan",
                    "transfer_size_gb": 4.7,
                    "detection_rule": "det-cnf-002",
                },
            },
            {
                "title": "UEBA: Exam coordinator account accessing 50,000+ records/hour (baseline: 200/hour)",
                "severity": "high",
                "source": "AiSOC UEBA",
                "category": "ueba",
                "sourcetype": "aisoc:ueba:anomaly",
                ...
                "extra": {
                    "user": "exam.coordinator@cbse.gov.in",
                    "records_per_hour": 50000,
                    "baseline_records_per_hour": 200,
                    "deviation_score": 8.7,
                    "anomaly_type": "anomalous_data_access_volume",
                    "peer_group": "cbse_exam_coordinators",
                },
            },
        ],
        "playbook_run": {
            "playbook_id": "cni-education-apt-response-v1",
            "playbook_name": "CNI Education: APT Breach Response (CBSE Pattern)",
            "status": "running",
            "context": {"incident_severity": "critical", "actor": "APT36", "sector": "education_cni"},
            "steps": [
                ("attribute_apt", "India APT attribution — APT36 confirmed (0.87)", "completed", {"actor": "APT36", "confidence": 0.87}),
                ("protect_exam_data", "Emergency read-only lock on exam result databases", "completed", {"databases_locked": 3}),
                ("revoke_compromised_creds", "Force-expire compromised coordinator credentials", "completed", {"users_reset": 1}),
                ("block_c2_infra", "Block APT36 Crimson RAT C2 infrastructure", "completed", {"ips_blocked": 7, "domains_blocked": 12}),
                ("certin_report", "File CERT-In CAT-9 data breach report (6-hour deadline)", "completed", {"report_id": "CERTIN-CNI002-C3D4", "submitted": True}),
                ("notify_moe", "Notify Ministry of Education", "running", {}),
                ("threat_hunt", "90-day APT36 dwell-time threat hunt", "pending", {}),
            ],
        },
    },
    {
        "key": "CNI-003",
        "title": "Volt Typhoon pre-positioning in India power grid OT environment",
        "description": (
            "China-linked Volt Typhoon group detected using living-off-the-land techniques to "
            "map NTPC/PGCIL substation automation network. No malware deployed — pure LOTL "
            "tradecraft using native Windows tools to enumerate OT DMZ. Modbus unauthorized "
            "write commands detected targeting substation PLC register 4096. "
            "NCIIPC mandatory notification triggered under IT Act 2000 Section 70."
        ),
        "severity": "critical",
        "status": "open",
        "host": "PGCIL-SUBSTATION-HMI01",
        "user": "ot-admin@pgcil.gov.in",
        "src_ip": "10.100.0.99",
        "tactic_ids": ["TA0043", "TA0007", "TA0008"],
        "technique_ids": ["T1016", "T1049", "T1082", "T1021.001"],
        "tags": ["volt_typhoon", "ot", "power_grid", "cni", "nciipc", "modbus", "scada", "living_off_land"],
        "alerts": [
            {
                "title": "OT Connector: Modbus unauthorized FC-16 write to PLC register 4096 from IT-side IP (PGCIL-SUBSTATION)",
                "severity": "critical",
                "source": "AiSOC OT Connector",
                "category": "ot",
                "sourcetype": "aisoc:ot:modbus",
                "process": "modbus_monitor",
                "ai_score": 0.99,
                "extra": {
                    "protocol": "modbus",
                    "function_code": 16,
                    ...
                    "asset_type": "plc",
                    "mitre_ics_techniques": ["T0855", "T0831", "T0836"],
                    "ot_safe_response": True,
                    "detection_rule": "det-ot-001",
                },
            },
            {
                "title": "Volt Typhoon LOTL: 8 native recon commands in 3 minutes from OT workstation (baseline: 0)",
                "severity": "high",
                "source": "AiSOC India APT Agent",
                "category": "endpoint",
                "sourcetype": "aisoc:apt:volt_typhoon",
                "process": "cmd.exe",
                "ai_score": 0.93,
                "extra": {
                    "actor": "Volt_Typhoon",
                    "technique": "Living-off-the-land",
                    "commands_executed": ["netsh", "ipconfig /all", "net view", "tracert", "ping", "nslookup", "route print", "net user"],
                    "time_window_seconds": 180,
                    "host_role": "ot_workstation",
                    "detection_rule": "det-iapt-003",
                    "nciipc_ref": "NCIIPC-2023-009",
                },
            },
            {
                "title": "OT Risk Agent: SAFETY_CRITICAL impact classification — potential loss of grid control",
                "severity": "critical",
                "source": "AiSOC OT Risk Agent",
                "category": "ot",
                "sourcetype": "aisoc:ot:risk",
                "process": "ot_risk_assessor",
                "ai_score": 0.97,
                "extra": {
                    "impact_level": "SAFETY_CRITICAL",
                    "ics_mitre_techniques": ["T0855", "T0831", "T0826", "T0827"],
                    "ot_safe_response": True,
                    "isolation_prohibited": True,
                    "recommended_action": "network_dmz_segmentation_only",
                    "nciipc_notification_required": True,
                },
            },
        ],
        "playbook_run": {
            "playbook_id": "cni-power-grid-ot-response-v1",
            "playbook_name": "CNI Power Grid: OT/ICS Attack Response",
            "status": "running",
            "context": {"incident_severity": "critical", "actor": "Volt_Typhoon", "sector": "power_grid_cni"},
            "steps": [
                ("ot_classify", "Classify OT asset criticality — SAFETY_CRITICAL", "completed", {"impact": "SAFETY_CRITICAL", "process_state": "running"}),
                ("ot_network_segment", "Apply IT-OT DMZ firewall rule (OT-SAFE: no host isolation)", "completed", {"rule_applied": True, "ot_process_preserved": True}),
                ("alert_ot_team", "Alert OT Security Team and Grid Control Centre (POSOCO)", "completed", {"notified": ["ot_security", "grid_control", "ciso"]}),
                ("nciipc_notify", "NCIIPC mandatory notification (IT Act 2000 Section 70)", "running", {"report_id": "CERTIN-CNI003-E5F6", "agency": "NCIIPC"}),
                ("block_volt_typhoon_c2", "Block Volt Typhoon C2 and FRP proxy patterns", "pending", {}),
                ("historian_backup", "Emergency offline backup of process historian", "pending", {}),
                ("manual_gate", "HUMAN APPROVAL: Physical grid protection measures", "pending", {}),
            ],
        },
    },
    {
        "key": "CNI-004",
        "title": "EoL Windows 7 exploitation in government ministry — EternalBlue lateral spread",
        "description": (
            "Threat actor exploited CVE-2017-0144 (EternalBlue) on unpatched Windows 7 workstations "
            "in a state government IT department. 67% of the ministry's 320 workstations run "
            "EoL Windows 7 (no patches since 2020). Original CVSS 7.5 amplified to 16.5 due to "
            "permanent EoL status. Attacker pivoted to Active Directory, created backdoor admin account."
        ),
        "severity": "critical",
        "status": "investigating",
        "host": "MINISTRY-WS-047",
        "user": "user047@ministry.gov.in",
        "src_ip": "198.51.100.77",
        "tactic_ids": ["TA0001", "TA0004", "TA0008"],
        "technique_ids": ["T1190", "T1068", "T1136.001", "T1021.002"],
        "tags": ["eol_windows7", "eternalblue", "government", "cni", "eol_amplification", "certin_cat3"],
        "alerts": [
            {
                "title": "EoL-Amplified Alert: CVE-2017-0144 EternalBlue on Windows 7 — CVSS 7.5 → 16.5 (×2.2 EoL amplifier)",
                "severity": "critical",
                "source": "AiSOC EoL Prioritiser",
                "category": "vulnerability",
                "sourcetype": "aisoc:eol:amplification",
                ...
                "extra": {
                    ...
                    "amplified_cvss": 16.5,
                    "amplifier": 2.2,
                    "eol_workstations_affected": 214,
                    "total_workstations": 320,
                    "eol_percentage": 66.9,
                    "patch_available": False,
                },
            },
            {
                "title": "WinEventLog: New local admin account 'svc_maint' created outside change window",
                "severity": "critical",
                "source": "Windows Security",
                "category": "siem",
                "sourcetype": "WinEventLog:Security",
                ...
                "extra": {
                    "EventCode": 4720,
                    "NewAccountName": "svc_maint",
                    "SubjectUserName": "MINISTRY-WS-047$",
                    "mitre_technique": "T1136.001",
                    "change_window_active": False,
                },
            },
            {
                "title": "UEBA: Ministry workstation account accessing AD controller outside business hours (deviation 9.2)",
                "severity": "high",
                "source": "AiSOC UEBA",
                "category": "ueba",
                "sourcetype": "aisoc:ueba:anomaly",
                ...
                "extra": {
                    "user": "user047@ministry.gov.in",
                    "anomaly_type": "off_hours_privileged_access",
                    "access_time": "02:47 IST",
                    "baseline_access_hours": "09:00-18:00 IST",
                    "deviation_score": 9.2,
                    "target": "DC01.ministry.gov.in",
                },
            },
        ],
        "playbook_run": {
            "playbook_id": "cni-govt-it-breach-response-v1",
            "playbook_name": "CNI Government IT: APT Breach & Data Theft Response",
            "status": "running",
            "context": {"incident_severity": "critical", "sector": "government_cni", "eol_context": True},
            "steps": [
                ("classify_data", "Classify data sensitivity — SECRET level data at risk", "completed", {"classification": "SECRET"}),
                ("revoke_creds", "Disable backdoor account svc_maint + force reset user047", "completed", {"accounts_disabled": 1, "accounts_reset": 1}),
                ("isolate_host", "Isolate compromised Ministry workstation", "completed", {"host": "MINISTRY-WS-047"}),
                ("certin_report", "File CERT-In CAT-3 unauthorized access report", "completed", {"report_id": "CERTIN-CNI004-G7H8"}),
                ("enable_enhanced_logging", "Enable enhanced audit across ministry network", "running", {}),
                ("apt_hunt", "90-day dwell time hunt — EternalBlue propagation scope", "pending", {}),
                ("notify_leadership", "Notify Ministry CISO and NCIIPC", "pending", {}),
            ],
        },
    },
]
